...
async function generateSubchapter(chapter, subchapter, pathKey, choiceHistory = [], options = {}) {
  ...
  const generationPromise = (async () => {
    ...
    // These are singleton-scoped because prompt assembly methods read from `this`.
    // Set them only after acquiring the sequential generation slot so queued
    // prefetch jobs cannot overwrite each other's prompt context.
